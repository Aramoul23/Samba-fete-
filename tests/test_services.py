...
from app.models import db, Event, Client, Payment, EventLine
...
class TestDateConflict:
    def test_no_conflict_on_empty_db(self, _db):
        errors = BookingService.validate_date_conflict("2026-06-15")
        assert errors == []

    def test_conflict_with_confirmed_event(self, _db):
    ...
    def test_conflict_with_pending_event(self, _db):
        c = Client(name="Test", phone="123")
        db.session.add(c); db.session.flush()
    ...
    def test_no_conflict_with_cancelled_event(self, _db):
        c = Client(name="Test", phone="123")
        db.session.add(c); db.session.flush()
    ...
    def test_exclude_own_id(self, _db):
        c = Client(name="Test", phone="123")
        db.session.add(c); db.session.flush()
...
class TestCreateEvent:
    def test_create_valid_event(self, _db):
        data = ImmutableMultiDict([
            ("title", "Test Wedding"), ("client_name", "Ahmed"),
            ("client_phone", "0555123456"), ("venue_id", "1"),
            ("event_date", "2026-08-01"), ("time_slot", "Soirée"),
            ("total_amount", "300000"), ("deposit_required", "50000"),
        ])
        event, errors = BookingService.create_event(data)
        assert errors == []
        assert event is not None
        assert event.total_amount == 300000

    def test_create_event_missing_title(self, _db):
        data = ImmutableMultiDict([
            ("title", ""), ("client_name", "Ahmed"),
            ("venue_id", "1"), ("event_date", "2026-08-01"),
        ])
        event, errors = BookingService.create_event(data)
        assert event is None
        assert len(errors) > 0

    def test_create_event_creates_deposit_payment(self, _db):
        data = ImmutableMultiDict([
            ("title", "With Deposit"), ("client_name", "Test"),
            ("client_phone", "123"), ("venue_id", "1"),
            ("event_date", "2026-09-01"), ("time_slot", "Soirée"),
            ("total_amount", "200000"), ("deposit_required", "75000"),
        ])
        event, errors = BookingService.create_event(data)
        assert errors == []
...
class TestPayments:
    ...
    def test_payment_zero_amount(self, _db):
        event = self._create_event()
        _, error = BookingService.add_payment(event.id, 0)
        assert error is not None

    def test_full_payment_auto_confirms(self, _db):
        event = self._create_event()
    ...
    def test_partial_payment_keeps_pending(self, _db):
        event = self._create_event()
...
class TestFinancials:
    def test_get_financials_empty(self, _db):
        c = Client(name="Test", phone="123")
        db.session.add(c); db.session.flush()
        e = Event(title="Test", client_id=c.id, venue_id=1,
    ...
    def test_get_financials_with_data(self, _db):
        c = Client(name="Test", phone="123")
        db.session.add(c); db.session.flush()
        e = Event(title="Test", client_id=c.id, venue_id=1,
                  event_type="Mariage", event_date="2026-11-01", time_slot="Soirée",
