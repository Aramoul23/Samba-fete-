...
def can_delete():
    """Vérifie si l'utilisateur actuel peut supprimer des éléments."""
    return is_admin()


def can_export():
    """Vérifie si l'utilisateur actuel peut exporter des données."""
...
def _get_event_financials_db(db, event_id):
    ...
    # Get event total
    event = db.execute(
        "SELECT total_amount FROM events WHERE id=?", (event_id,)
    ).fetchone()
...
def index():
    ...
    total_events = db.execute(
        "SELECT COUNT(*) as c FROM events WHERE status != 'annulé'"
    ).fetchone()["c"]
    ...
    last_month_revenue = db.execute(
        ...
        "JOIN events e ON p.event_id=e.id "
        "WHERE p.payment_date BETWEEN ? AND ? AND e.status != 'annulé' AND p.is_refunded=0",
        (prev_first.isoformat(), prev_last.isoformat()),
    ).fetchone()["s"]
    ...
    if float(last_month_revenue) > 0:
        revenue_pct_change = (
            (float(revenue_month) - float(last_month_revenue))
...
def event_form(event_id=None):
    ...
    if request.method == "POST":
        ...
        if event_date:
            conflict = db.execute(
                "SELECT e.id, e.title, e.status, COALESCE(v.name, '') as venue_name FROM events e "
            ...
            if conflict:
                ...
                if conflict['status'] == 'confirmé':
                    errors.append(f"⛔ Date déjà réservée! '{conflict['title']}' est prévu le {event_date}{venue_label} — 🔒 Cette date est verrouillée")
                else:
                    errors.append(f"⚠️ Attention! '{conflict['title']}' est en attente pour le {event_date}{venue_label} — La validation est en cours")
        ...
        if event:
            db.execute(
                "UPDATE events SET title=?, client_id=?, venue_id=?, venue_id2=?, event_type=?, event_date=?, "
            ...
            cur = db.execute(
                "INSERT INTO events (title, client_id, venue_id, venue_id2, event_type, event_date, "
        ...
        if price_location > 0:
            db.execute(
                "INSERT INTO event_lines (event_id, description, amount, is_cost) VALUES (?,?,?,0)",
        ...
        for i, desc in enumerate(line_descs):
            if desc.strip():
                ...
                db.execute(
                    "INSERT INTO event_lines (event_id, description, amount, is_cost) VALUES (?,?,?,?)",
                    (event_id, desc.strip(), amount, is_cost),
                )

        db.commit()
        flash(
            "Événement créé avec succès! Vous pouvez maintenant encaisser un paiement.",
            "success",
        )
...
def event_detail(event_id):
    """Affiche les détails d'un événement avec ses finances."""
    db = get_db_connection()
    event = db.execute(
...
@app.route(
    "/evenement/<int:event_id>/paiement/<int:payment_id>/rembourser", methods=["POST"]
)
@login_required
        ...
        for cat_key, cat_name, default_price in [
            ...
            if request.form.get(f"cat_{cat_key}"):
                amount = request.form.get(
                    f"amount_{cat_key}", default_price, type=float
                )
                if amount > 0:
                    db.execute(
...
@login_required
def event_list():
    """Affiche la liste filtrable des événements."""
    db = get_db_connection()
...
@login_required
def financials():
    """Affiche le rapport financier avec revenus et bénéfices."""
    db = get_db_connection()
...
@login_required
def client_list():
    """Affiche la liste des clients avec recherche."""
    db = get_db_connection()
    search = request.args.get("q", "").strip()
...
def client_detail(client_id):
    """Affiche les détails d'un client avec son historique."""
    db = get_db_connection()
...
def expenses():
    ...
    expenses_by_category = [
        ...
        for r in expenses_by_category_raw
    ]
...
def accounting():
    ...
    while current <= end:
        ...
        if current.month == 12:
            ...
            month_end = current.replace(month=current.month + 1, day=1) - timedelta(
                days=1
            )
...
def delete_venue(venue_id):
    """Supprime un lieu s'il n'a pas d'événements associés."""
    db = get_db_connection()
    count = db.execute(
...
@login_required
def users():
    """Affiche la gestion des utilisateurs (admin uniquement)."""
    if not is_admin():
...
def api_calendar_events():
    """API retournant les événements du calendrier au format JSON."""
    db = get_db_connection()
    year = request.args.get("year", date.today().year, type=int)
    month = request.args.get("month", date.today().month, type=int)
...
def export_finances():
    ...
    export_date = datetime.now().strftime("%Y-%m-%d %H:%M")
    ods_content = export_financials_ods(event_financials, summary_stats, export_date)

    response = make_response(ods_content)
    response.headers["Content-Type"] = "application/vnd.oasis.opendocument.spreadsheet"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=finances_{start_date}_{end_date}.ods"
    )
    return response
...
def export_expenses():
    ...
    response.headers["Content-Type"] = "application/vnd.oasis.opendocument.spreadsheet"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=depenses_{start_date}_{end_date}.ods"
    )
    return response
...
def export_pl():
    ...
    response.headers["Content-Type"] = "application/vnd.oasis.opendocument.spreadsheet"
    response.headers["Content-Disposition"] = (
        f"attachment; filename=rapport_pl_{start_date}_{end_date}.ods"
    )
    return response
