...
def export_events_ods(events, export_date):
    ...
    for i, event in enumerate(events, 1):
        invited = (event.get("guests_men", 0) or 0) + (
            event.get("guests_women", 0) or 0
        )
    ...
    for i, client in enumerate(clients, 1):
        ...
        rows.append(
            [
                ...
                client.get("name", ""),
                client.get("phone", ""),
                client.get("phone2", "") or "",
                client.get("email", "") or "",
                client.get("address", "") or "",
                client.get("event_count", 0) or 0,
                billed,
                paid,
                billed - paid,
            ]
...
def export_financials_ods(event_financials, summary_stats, export_date):
    ...
    for i, ef in enumerate(event_financials, 1):
        ...
        rows.append(
            [
                ...
                revenue - costs,
                paid,
                float(ef.get("total_amount", 0) or 0) - paid,
                ef.get("status", ""),
            ]
        )
    ...
    totals = {
        5: total_revenue,
        6: total_costs,
        7: total_revenue - total_costs,
        8: total_paid,
        9: total_revenue - total_paid,
    }
...
def export_pl_report_ods(monthly_data, export_date):
    ...
    for month in monthly_data:
        ...
        rows.append(
            [month.get("month", ""), income, expenses, profit, f"{margin:.1f}%"]
        )
